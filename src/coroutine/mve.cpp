...
static void CallCurrentCoroutine() { current_coroutine->Call(); }
...
Coroutine::Coroutine(std::function<void()> bound_fn)
    : status(Status::kSuspended),
      bound_fn_(bound_fn),
      stack_(
          new (kStackAlignmentBytes)
              coroutine_stack_t[kStackSizeBytes / sizeof(coroutine_stack_t)]),
  ...
  stack_top_[kLinkRegisterOffset] =
      reinterpret_cast<coroutine_stack_t>(CallCurrentCoroutine);
}
